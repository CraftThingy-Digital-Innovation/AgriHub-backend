...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.alterTable('users', (t) => {
    if (!cols.username) t.string('username').nullable();
    if (!cols.email_verified) t.boolean('email_verified').defaultTo(false);
    if (!cols.email_verify_token) t.string('email_verify_token').nullable();
    if (!cols.email_verify_expires) t.string('email_verify_expires').nullable();
    if (!cols.phone_verified) t.boolean('phone_verified').defaultTo(false);
    if (!cols.phone_otp) t.string('phone_otp').nullable();       // 6-digit OTP
    if (!cols.phone_otp_expires) t.string('phone_otp_expires').nullable();
  });
