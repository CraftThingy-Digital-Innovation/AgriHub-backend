  ...
  try {
    ...
    if (isCommand) {
      if (cleanUpper.startsWith('VERIFIKASI ')) {
        ...
        await db('users').where({ id: linkUser.id }).update({
          whatsapp_lid: sender,
          phone_verified: true,
    ...
    if (isGroup) {
      ...
      if (!user) {
        ...
        if (foundByPhone) {
          ...
          await sendWAMessage(jid,
            `👋 *Halo!* Sepertinya Anda belum terdaftar di AgriHub.\n\n` +
      ...
      if (user.whatsapp_lid !== sender && sender.endsWith('@lid')) {
        await db('users').where({ id: user.id }).update({ whatsapp_lid: sender, updated_at: new Date().toISOString() });
