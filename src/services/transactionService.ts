...
export async function createOrderFromMatch(input: OrderInput) {
    ...
    const match = await db('match_history')
        ...
        .select(
            'match_history.*',
            'supply_reports.reporter_id as seller_id',
            'supply_reports.komoditas',
            'supply_reports.harga_per_kg as unit_price',
            'supply_reports.jumlah_kg as current_stock_kg',
            'demand_requests.jumlah_kg as requested_qty'
        )
        .first();
    ...
    if (qty <= 0) throw new Error('Stok tidak tersedia untuk pesanan ini');
...
export async function updateShippingStatus(orderId: string, courier: string, resi: string) {
...
export async function runAutoConfirmJob() {
    ...
    const ordersToConfirm = await db('orders')
