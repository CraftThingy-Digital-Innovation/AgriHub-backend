...
router.get('/stats', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const storeStatsRaw = await db('orders')
    ...
    const dailyTrendsRaw = await db('orders')
...
router.patch('/:id/status', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (status === 'dibatalkan' && !['pending', 'menunggu_bayar'].includes(order.status)) {
      res.status(400).json({ success: false, error: 'Pesanan tidak bisa dibatalkan karena sudah dibayar/diproses' }); return;
    }
