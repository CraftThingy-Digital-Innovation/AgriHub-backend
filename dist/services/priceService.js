...
async function searchCommodityPrices(query) {
    ...
    try {
        ...
        const rows = await (0, knex_1.default)('pihps_prices').where('date', date).distinct('commodity_name');
        const allCommodities = rows.map((r) => String(r.commodity_name).toLowerCase().trim());
        ...
        const matchedCommodities = allCommodities.filter(comm => queryWords.some(word => comm.includes(word) || word.includes(comm)));
        if (matchedCommodities.length === 0)
            return "";
        // 5. Tarik data berdasarkan komoditas-komoditas yang cocok
        const prices = await (0, knex_1.default)('pihps_prices')
            .where('date', date)
            .whereIn('commodity_name', rows
            .filter((r) => matchedCommodities.includes(String(r.commodity_name).toLowerCase().trim()))
            .map((r) => r.commodity_name))
        ...
        const provRows = await (0, knex_1.default)('pihps_prices').distinct('prov_name');
        const allProvinces = provRows.map((r) => String(r.prov_name).toLowerCase());
        ...
        if (targetedProvince) {
            ...
            if (filtered.length > 0)
                dataToFormat = filtered;
        }
        else {
            // Jika tidak sebut provinsi, hitung AGREGAT RATA-RATA NASIONAL (per komoditas)
            const natAvg = {};
            prices.forEach(p => {
                if (!natAvg[p.commodity_name])
                    natAvg[p.commodity_name] = { sum: 0, count: 0 };
        ...
        console.error('❌ PIHPS DB Search Error:', error.message);
