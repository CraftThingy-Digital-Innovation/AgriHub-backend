...
async function searchCommodityPrices(query) {
    ...
    try {
        ...
        const rows = await (0, knex_1.default)('pihps_prices').distinct('commodity_name');
        ...
        for (const comm of allCommodities) {
            ...
            let isMatch = false;
            if (lowerQuery.includes(baseWord))
                isMatch = true;
            // Handle street alias
            else if (['cabe', 'lombok'].some(w => lowerQuery.includes(w)) && baseWord === 'cabai')
                isMatch = true;
            else if (lowerQuery.includes('brambang') && baseWord === 'bawang')
                isMatch = true;
            else if (['padi', 'gabah'].some(w => lowerQuery.includes(w)) && baseWord === 'beras')
                isMatch = true;
            if (isMatch)
                matchedBaseWords.add(baseWord);
        }
