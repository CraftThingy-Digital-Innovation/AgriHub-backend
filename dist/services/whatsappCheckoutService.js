...
exports.processBeliCommand = processBeliCommand;
exports.processPilihKurirCommand = processPilihKurirCommand;
...
async function processBeliCommand(jid, senderLid, matchId) {
    try {
        const user = await (0, knex_1.default)('users').where({ whatsapp_lid: senderLid }).first();
        if (!user) {
            await (0, whatsappBot_1.sendWAMessage)(jid, '❌ Akun Anda belum terhubung. Ketik *LINK [NomorHP]* terlebih dahulu.');
            return;
        }
        const match = await (0, knex_1.default)('match_history')
            ...
            .select('match_history.*', 'products.name as product_name', 'products.price_per_unit', 'products.weight_gram', 'stores.id as store_id', 'stores.name as store_name', 'stores.postal_code as store_postal', 'user_addresses.id as address_id', 'user_addresses.postal_code as buyer_postal', 'demand_requests.jumlah_kg').first();
        if (!match) {
            await (0, whatsappBot_1.sendWAMessage)(jid, '❌ Data matching tidak ditemukan atau sudah kadaluarsa.');
            return;
        }
        if (!match.store_postal || !match.buyer_postal) {
            await (0, whatsappBot_1.sendWAMessage)(jid, '❌ Kode pos penjual atau pembeli belum lengkap untuk perhitungan ongkir otomatis. Mohon perbarui di dashboard.');
            return;
        }
        await (0, whatsappBot_1.sendWAMessage)(jid, '⏳ Sedang menghitung opsi ongkos kirim (Biteship)...');
        const totalWeightGram = Number(match.jumlah_kg) * 1000;
        const rates = await (0, biteshipService_1.checkOngkir)({
        ...
        if (rates.length === 0) {
            await (0, whatsappBot_1.sendWAMessage)(jid, '❌ Gagal menemukan kurir untuk rute pengiriman ini.');
            return;
        }
        ...
        await (0, knex_1.default)('whatsapp_auth').insert({
            id: (0, uuid_1.v4)(),
        ...
        await (0, whatsappBot_1.sendWAMessage)(jid, '❌ Terjadi kesalahan pada sistem pemrosesan checkout.');
    }
}
async function processPilihKurirCommand(jid, senderLid, selection) {
    try {
        const stateId = await getUserState(senderLid, 'pending_checkout');
        if (!stateId) {
            await (0, whatsappBot_1.sendWAMessage)(jid, '❌ Tidak ada transaksi aktif yang sedang menunggu pemilihan kurir. Ketik *BELI [ID Match]* terlebih dahulu.');
            return;
        }
        const stateRow = await (0, knex_1.default)('whatsapp_auth').where({ category: 'checkout_state', key_id: stateId }).first();
        if (!stateRow) {
            await clearUserState(senderLid, 'pending_checkout');
            await (0, whatsappBot_1.sendWAMessage)(jid, '❌ Sesi checkout Anda sudah kadaluarsa (lebih dari 1 jam). Ulangi kembali ya.');
            return;
        }
        ...
        if (idx < 0 || idx >= rates.length) {
            await (0, whatsappBot_1.sendWAMessage)(jid, `❌ Pilihan nomor ${selection} tidak valid. Mohon masukkan angka 1 hingga ${rates.length}.`);
            return;
        }
        const selectedRate = rates[idx];
        await (0, whatsappBot_1.sendWAMessage)(jid, `⏳ Memproses Invoice menggunakan ${selectedRate.courier.toUpperCase()}...`);
        // Dapatkan data produk asli untuk kalkulasi
        const prod = await (0, knex_1.default)('products').where('id', product_id).first();
        ...
        const mdLink = await (0, transactionService_1.createTransactionMidtrans)(orderId, finalGrandTotal);
        await (0, knex_1.default)('orders').insert({
            id: orderId,
            buyer_id: user_id,
            seller_id: (await (0, knex_1.default)('stores').where('id', store_id).first()).owner_id,
            store_id: store_id,
        ...
        await clearUserState(senderLid, 'pending_checkout');
        await (0, knex_1.default)('whatsapp_auth').where({ category: 'checkout_state', key_id: stateId }).delete(); // Cleanup
        ...
        await (0, whatsappBot_1.sendWAMessage)(jid, invoiceStr);
    }
    catch (err) {
        console.error('Pilih कुरिर Error:', err);
        await (0, whatsappBot_1.sendWAMessage)(jid, '❌ Terjadi kesalahan pada saat menyelesaikan invoice.');
    }
}
// Simple state helpers
async function saveUserState(lid, stateKey, payloadStr) {
    const keyId = `${lid}_${stateKey}`;
    const exist = await (0, knex_1.default)('whatsapp_auth').where({ category: 'user_state', key_id: keyId }).first();
    if (exist) {
        await (0, knex_1.default)('whatsapp_auth').where({ id: exist.id }).update({ data: payloadStr, updated_at: new Date().toISOString() });
    }
    else {
        await (0, knex_1.default)('whatsapp_auth').insert({ id: (0, uuid_1.v4)(), category: 'user_state', key_id: keyId, data: payloadStr });
    }
}
async function getUserState(lid, stateKey) {
    const keyId = `${lid}_${stateKey}`;
    const row = await (0, knex_1.default)('whatsapp_auth').where({ category: 'user_state', key_id: keyId }).first();
    return row ? row.data : null;
}
async function clearUserState(lid, stateKey) {
    const keyId = `${lid}_${stateKey}`;
    await (0, knex_1.default)('whatsapp_auth').where({ category: 'user_state', key_id: keyId }).delete();
}
//# sourceMappingURL=whatsappCheckoutService.js.map