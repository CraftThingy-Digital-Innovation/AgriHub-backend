...
router.post('/report', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (!komoditas_id || !price_per_kg || !kabupaten || !provinsi) {
            res.status(400).json({ success: false, error: 'komoditas_id, price_per_kg, kabupaten, provinsi wajib' });
            return;
        }
        ...
        res.status(500).json({ success: false, error: 'Gagal lapor harga' });
    }
});
// ─── GET /api/price/history/:komoditasId — Historis 30 hari ──────────────
router.get('/history/:komoditasId', async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Gagal fetch histori' });
    }
});
// ─── GET /api/price/predict/:komoditasId — Prediksi AI 2 minggu ──────────
router.get('/predict/:komoditasId', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const cached = await (0, knex_1.default)('price_predictions')
            ...
            .first();
        if (cached) {
            res.json({ success: true, data: JSON.parse(cached.predictions_json), cached: true });
            return;
        }
        ...
        res.status(500).json({ success: false, error: 'Gagal generate prediksi' });
    }
});
// ─── POST /api/price/alert — Set alert harga ─────────────────────────────
router.post('/alert', auth_1.requireAuth, async (req, res) => {
    try {
        const { komoditas_id, alert_type, threshold_price, provinsi } = req.body;
        if (!komoditas_id || !alert_type || !threshold_price) {
            res.status(400).json({ success: false, error: 'komoditas_id, alert_type, threshold_price wajib' });
            return;
        }
        ...
        res.status(500).json({ success: false, error: 'Gagal buat alert' });
    }
});
